        ...
        for idx, line in enumerate(f):
            ...
            for idx, c in enumerate(others):
                ...
                if closed_profit_byATR and one_day_before_entry_closed:
                    ...
                    tmp_dict[closed_str_date] = convert_jd(
                        one_day_before_entry_closed)
...
def cal_profit(df, riskR=0.003):
    ...
    for e in list(df.columns):
        ...
        if e[:len_of_nums] == 'closed_profit_byATR':
            sel_cols.append(e)
        if e[:len_of_nums] == 'day_before_entry':
            sel_cols1.append(e)
...
if __name__ == '__main__':
    ...
    # df.to_csv('./result/portfolio-test-1.csv')
    # pnl1.to_csv('./result/portfolio-test-1-r.csv')
# %% 2.比較兩種價格圖
    ...
    # pnl2.to_csv('./result/portfolio-test-2-r.csv',header=None)
# %% 3. 讀取資料夾底下(read all PL-*.txt )=> 測試四種價格圖
    ...
    # pnl_tot.to_csv('./result/portfolio-test-tot-r.csv',header=None)
